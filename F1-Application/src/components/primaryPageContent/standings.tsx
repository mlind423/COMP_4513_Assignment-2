                ...
                <ul className="SM-leftStanding">
                    ...
                    {props.data.drivers.map((c:any, index:number) => {
                        ...
                        <li key={index}>
                            ...
                            <button id="driver" className="text-secondary-content btn btn-secondary btn-sm rounded-full m-2 border-transparent text-secondary-content border-0  hover:btn-primary hover:text-primary-content" value={c.drivers.driverRef} onClick={handleModal}>{c.drivers.forename + " " + c.drivers.surname}</button>
                ...
                <ul className="SM-rightStanding">
                    ...
                    {props.data.constructors.map((c:any, index:number) => {
                        ...
                        <li key={index}>
                            <div className="SM-Position">{c.position}</div>
                            {/* <Modal text={c.constructors.name} type='constructor' key={c.constructors.constructorRef} refId={c.constructors.constructorRef} test="test"/> */}
                            <button id="constructor" className="text-secondary-content btn btn-secondary btn-sm rounded-full m-2 border-transparent text-secondary-content border-0  hover:btn-primary hover:text-primary-content" value={c.constructors.constructorRef} onClick={handleModal}>{c.constructors.name}</button>
                            <div className="SM-Points">{c.points}</div>
                            <div className="SM-Wins">{c.wins}</div>
                        </li>)
                    })}
